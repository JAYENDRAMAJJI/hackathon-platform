  ...
  const fetchData = async () => {
    ...
    try {
      const [tcResp, qResp] = await Promise.all([
        apiClient.get('/admin/test-cases', selectedQuestionId !== 'ALL' ? { questionId: selectedQuestionId } : undefined),
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-white tracking-tight flex items-center gap-2.5">
            <CheckSquare className="w-6 h-6 text-purple-400" /> Test Suite & Sandbox Validation Cases
          </h1>
      ...
      <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl shadow-xl flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3 w-full md:w-auto">
          <span className="text-xs font-bold text-slate-400">Filter by Problem:</span>
          <select
            value={selectedQuestionId}
            onChange={(e) => setSelectedQuestionId(e.target.value)}
            className="px-3.5 py-2 bg-slate-800 border border-slate-700 rounded-xl text-xs text-white focus:outline-none focus:border-purple-500 max-w-md truncate"
          >
