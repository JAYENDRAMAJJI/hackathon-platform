...
app.get("/api/student/contest/state", authenticateStudent, (req, res) => {
  ...
  let targetQuestions = (contest.questionIds || []).map((qId) => db.questions.find((q) => q.id === qId && q.status === "ACTIVE")).filter(Boolean);
  ...
  const visibleTests = db.testCases.filter((tc) => tc.questionId === activeQuestion.id && !tc.isHidden && tc.isEnabled).map((tc) => ({
    id: tc.id,
    input: tc.input,
  ...
  res.json({
    ...
    data: {
      ...
      question: {
        ...
        inputFormat: activeQuestion.inputFormat || "Line 1: Input arguments formatted per signature",
        outputFormat: activeQuestion.outputFormat || "Expected evaluated return value",
...
app.post("/api/student/submit-code", authenticateStudent, (req, res) => {
  ...
  const recentStudentSubs = db.submissions.filter(
    (s) => s.studentId === student.id && Date.now() - new Date(s.submittedAt).getTime() < 3e4
  );
  if (recentStudentSubs.length >= 4) {
    ...
    db.anomalies.unshift(anomalyRecord);
  ...
  res.json({
    ...
    data: {
      ...
      rank: student.rank,
      message: `\u{1F389} Excellent Work! All 6 test cases passed. +${scoreAwarded} Points added to your score!`,
      time: `${execTimeMs} ms`,
      memory: memoryUsed,
